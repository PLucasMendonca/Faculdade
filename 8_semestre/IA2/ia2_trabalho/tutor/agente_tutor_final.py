...
"""
Este código implementa uma simplificação da lógica fuzzy do arquivo sti_prototype.py.
...
3. Se taxa de acerto em imagem é alta e as demais são baixas, então pFROM raulcsouza/spark-base-hadoop
...
ENTRYPOINT ["/bin/bash", "bootstrap.sh"]referência é imagem
...
def main():
    ...
    print("\n" + "*" * 70)
    print("*" + " " * 68 + "*")
    print("*  AGENTE TUTOR SIMPLIFICADO - MATRÍCULA: 2122130042" + " " * 18 + "*")
    print("*" + " " * 68 + "*")
    print("*" * 70)
    
    # Exemplos de perfis de alunos
    perfis = [
        {
            "nome": "ALUNO COM PREFERÊNCIA POR TEXTO",
            "dados": {
                "nu_acertos_texto": 25,
                "nu_erros_texto": 5,
        ...
        {
            "nome": "ALUNO COM PREFERÊNCIA POR IMAGEM",
            "dados": {
                "nu_acertos_texto": 12,
                "nu_erros_texto": 18,
                "nu_acertos_imagem": 28,
                "nu_erros_imagem": 2,
                "nu_acertos_video": 15,
                "nu_erros_video": 15
            }
        ...
        {
            "nome": "ALUNO COM PREFERÊNCIA POR VÍDEO",
            "dados": {
                "nu_acertos_texto": 8,
                "nu_erros_texto": 22,
    ...
    for perfil in perfis:
        ...
        print("\nDADOS DE ENTRADA:")
        print("-" * 50)
        print(f"Texto:  {dados['nu_acertos_texto']} acertos, {dados['nu_erros_texto']} erros")
        print(f"Imagem: {dados['nu_acertos_imagem']} acertos, {dados['nu_erros_imagem']} erros")
        print(f"Vídeo:  {dados['nu_acertos_video']} acertos, {dados['nu_erros_video']} erros")
        
        # Processar dados
        resultado = processar_dados(json.dumps(dados))
        
        # Exibir taxas de acerto
        print("\nTAXAS DE ACERTO:")
        print("-" * 50)
        ...
        print("-" * 50)
        print(f"Método preferido: {resultado['preferencia'].upper()}")
        
        # Exibir partes
        print("\nPARTES DE CONTEÚDO (JSON):")
        print("-" * 50)
        print(json.dumps(resultado["partes"], indent=2))
        
        # Salvar resultado em arquivo JSON
        nome_arquivo = f"resultado_{perfil['nome'].lower().replace(' ', '_')}.json"
        with open(nome_arquivo, 'w', encoding='utf-8') as f:
            json.dump(resultado["partes"], f, indent=2)
        
        print(f"\nResultado JSON salvo em: {nome_arquivo}")
